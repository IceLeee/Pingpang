...
- (BOOL)initDUTDevice:(int)index
{
    ...
    if ([_arrayDUTs count] > index) {
        if ([_arrCommDUT count] > index) {
        ...
        ORSSerialPort *tmp = [ORSSerialPort serialPortWithPath:_arrayDUTs[index]];
        if (tmp == nil)
        {
            if ([_strPortStatus length] > 0)
            {
...
- (BOOL)initFIXTUREDevice:(int)index
{
    ...
    if ([_arrayFIXTUREs count] > index) {
        ...
        ORSSerialPort *tmp = [ORSSerialPort serialPortWithPath:_arrayFIXTUREs[index]];
        if (tmp == nil)
        {
            if ([_strPortStatus length] > 0)
            {
                        ...
                        for (NSString *strTmpCmd in arrCommand) {
                            do {
                                ...
                                if ([strTmpCmd rangeOfString:@"CheckPudding"].length > 0) {
                                    ...
                                    //[self startRunShellScript:strPythoneFileName Content:@"cd /vault/tmp\npython ./b237Tool.py " Param:[NSString stringWithFormat:@"-disableBoost -setBoostOff %@", _arrayDUTs[indexFixture]]withIndex:indexFixture];
                                    [self startRunShellScript:strPythoneFileName Content:@"cd /Users/david/Desktop/tmp1\npython ./b237Tool.py " Param:[NSString stringWithFormat:@"-disableBoost -setBoostOff -serialPort %@", _arrayDUTs[indexFixture]]withIndex:indexFixture];
                                    if ([strResSpec length] > 0) {
                                        if ([strResult rangeOfString:strResSpec].length > 0) {
                                            bReturnStatus = YES;
                                        }
                                    ...
                                    //[self startRunShellScript:strPythoneFileName Content:@"cd /vault/tmp\npython ./b237Tool.py " Param:[NSString stringWithFormat:@"-serial -identify -max_timeout -ilimit 1500 -info -serialPort %@", _arrayDUTs[indexFixture]]withIndex:indexFixture];
                                    [self startRunShellScript:strPythoneFileName Content:@"cd /Users/david/Desktop/tmp1\npython ./b237Tool.py " Param:[NSString stringWithFormat:@"-serial -identify -max_timeout -ilimit 1500 -info -serialPort %@", _arrayDUTs[indexFixture]]withIndex:indexFixture];
                                    if ([strResSpec length] > 0) {
                                        if ([strResult rangeOfString:strResSpec].length > 0) {
                                            bReturnStatus = YES;
...
- (BOOL)queryBatman:(int)indexFixture
{
    BOOL bRet = NO;
    ...
    if ([_arrCommDUT[indexFixture] isOpen]) {
        //        [_arrCommDUT[indexFixture] write:@"[BATMAN]"];
        [self write:@"[BATMAN]" SerialPort:_arrCommDUT[indexFixture]];
        
        //        if ([_arrCommDUT[indexFixture] read:&result From:@"[BATMAN]<" To:@">" Timeout:3])
        if ([self read:&result From:@"[BATMAN]<" To:@">" Timeout:3 SerialPort:_arrCommDUT[indexFixture]])
        {
            if ([result length] > 0) {
                if (([result rangeOfString:@"255"].length > 0)) {
                    bRet = NO;
                } else {
                    bRet = YES;
                }
                
                [_arrTotalResBuffer[indexFixture] setString:[self getData:result startSet:@"[BATMAN]<" endSet:@">"]];
            }
        }
    }
...
#if ((DUT_TEST == 1) && (FIXTURE_TEST == 1))
- (BOOL)checkCaseFuse:(int)indexFixture
{
    if (_bAuditMode) {
        return YES;
    }
    ...
    [self closeDutPort:indexFixture];
    
    if ([_arrCommFIXTURE[indexFixture] isOpen]) {
        NSString *result = nil;
//        [_commFIXTURE1 write:@"#OFF*"];
        [self write:@"#OFF*" SerialPort:_arrCommFIXTURE[indexFixture]];
        
//        if ([_commFIXTURE1 read:&result From:@"#OFF" To:@"*" Timeout:5.0f])
        if ([self read:&result From:@"#OFF" To:@"*" Timeout:5.0f SerialPort:_arrCommFIXTURE[indexFixture]])
        {
            [self outputMessageTextView:result withIndex:indexFixture];
            [NSThread sleepForTimeInterval:5.0f];
//            [_commFIXTURE1 write:@"#SUP*"];
            [self write:@"#SUP*" SerialPort:_arrCommFIXTURE[indexFixture]];
//            if ([_commFIXTURE1 read:&result From:@"#SUP" To:@"*" Timeout:5.0f])
            if ([self read:&result From:@"#SUP" To:@"*" Timeout:5.0f SerialPort:_arrCommFIXTURE[indexFixture]])
            {
                [self outputMessageTextView:result withIndex:indexFixture];
                if ([self initDUTDevice:indexFixture]) {
                    [NSThread sleepForTimeInterval:1.0f];
//                    [_commDUT1 write:@"[LKS]"];
                    [self write:@"[LKS]" SerialPort:_arrCommDUT[indexFixture]];
                    
//                    if ([_commDUT1 read:&result From:@"<lks-00CC-" To:@">" Timeout:3.0f])
//                    if ([_commDUT1 read:&result From:@"<lks-33CC-" To:@">" Timeout:3.0f])
                    if ([self read:&result From:@"<lks-00CC-" To:@">" Timeout:3.0f SerialPort:_arrCommDUT[indexFixture]])
                    {
                        [self outputMessageTextView:result withIndex:indexFixture];
                        if ([result rangeOfString:@"1"].length > 0) {
                            bRet = YES;
                        } else {
                            [NSThread sleepForTimeInterval:1.0f];
//                            [_commDUT1 write:@"[LKS-33CC]"];
                            [self write:@"[LKS-33CC]" SerialPort:_arrCommDUT[indexFixture]];
                        }
                    } else {
                        [NSThread sleepForTimeInterval:1.0f];
//                        [_commDUT1 write:@"[LKS-33CC]"];
...
- (BOOL)OpenModemPort:(int)index
{
    if (_arrCommDUT[index] != nil) {
        ...
        while (times < 30) {
            ...
            if ([self isExistModemPort:_arrayDUTs[index]])
            {
